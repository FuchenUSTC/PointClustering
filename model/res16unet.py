...
from MinkowskiEngine import MinkowskiReLU, MinkowskiGlobalPooling, MinkowskiGlobalMaxPooling, MinkowskiLinear
...
class Res16Net(ResNetBase):
    ...
    def __init__(self,
               ...
               config,
               D=3):
        super(Res16Net, self).__init__(in_channels, out_channels, config, D)
        self.normalize_feature = config.net.normalize_feature
    ...
    def network_initialization(self, in_channels, out_channels, config, D):
        ...
        self.block4 = self._make_layer(
            ...
            norm_type=self.NORM_TYPE,
            bn_momentum=bn_momentum)
        self.pooling = MinkowskiGlobalMaxPooling()
    
        self.cls = MinkowskiLinear(self.PLANES[3], out_channels)
...
class Res16UNet34CMSG(Res16UNet34):
    PLANES = (32, 64, 128, 256, 256, 128, 96, 96)
    ...
    def __init__(self,
               ...
               config,
               D=3):
        super(Res16UNet34CMSG, self).__init__(in_channels, out_channels, config, D)
        self.normalize_feature = config.net.normalize_feature 

    def network_initialization(self, in_channels, out_channels, config, D):
      dilations = self.DILATIONS
      bn_momentum = config.opt.bn_momentum
      self.num_classes = config.net.model_n_out
      ...
      for i in range(self.num_classes):
          self.multi_shape_heads.append(
              nn.Sequential(
              conv(self.PLANES[7], self.PLANES[7],
                  kernel_size=space_n_time_m(config.net.head_kernel_size, 1),
                  stride=1, dilation=1,
             ...
             conv(self.PLANES[7], self.num_parts[i],
                  kernel_size=space_n_time_m(config.net.head_kernel_size, 1),
                  stride=1, dilation=1, conv_type=self.NON_BLOCK_CONV_TYPE, D=D)))
    ...
    def forward(self, x):
      ...
      logits_all_shapes = []
      for i in range(self.num_classes):
          logits_all_shapes.append(self.multi_shape_heads[i](out))
